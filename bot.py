...
async def on_guild_join(guild):
        channel = bot.get_channel(TCHANNEL)
...
async def submit(ctx, arg1, *, content='banana'):
...
#a lazy way of setting values to numbers cuz..... yea
        if progress == '1':
                ...
                prog="Finished Song"
        elif progress == str(numba):
                prog="Sick Chune"
        else:
                await channel.send("Please enter a valid track progress number, ~help for info")
...
#the if statement just makes sure the 1st parameter is in the range accepted and isn't used as an overflow vuln
        if progress == '1' or progress == '2' or progress == '3' or progress == str(numba):
...
#locks the cammand to a channel called 'feedback'
                ...
                if len(message.attachments) > 0 and message.channel.name =='feedback':
                        ...
                        await message.channel.send(f"Please make your submissions into links {message.author.mention}")
#deletes messages with links unless they are part of a command
                if 'http' in stuff:
                        if "~submit" in stuff or "~feedback" in stuff:
                                ...
                                await message.channel.purge(limit=1)
                                await message.channel.send(f'{message.author.mention} only links for submissions or feedback examples O.O, use the ~help command to see more.')
        await bot.process_commands(message)
